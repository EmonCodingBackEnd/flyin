...
import com.coding.flyin.starter.ftp.property.ReplayCode;
import com.coding.flyin.starter.ftp.result.*;
...
public class FTPTemplate implements FTPOperations {
    ...
    private void invalidateFTPClient(FTPClient ftpClient) {
        try {
            getFtpClientPool().invalidateObject(ftpClient);
        } catch (Exception e) {
            throw new FTPException(e);
        }
    }
    ...
    @Override
    public UploadResult uploadFile(UploadParam uploadParam) throws FTPException {
        ...
        try {
            ...
            for (Map.Entry<String, InputStream> entry :
                ...
                UploadResultItem resultItem =
                        uploadFile(ftpClient, remoteDirectory, entry.getKey(), entry.getValue());
            ...
            for (Map.Entry<String, String> entry : uploadParam.getContentMap().entrySet()) {
                ...
                UploadResultItem resultItem =
                        uploadFile(ftpClient, remoteDirectory, entry.getKey(), inputStream);
                uploadResult.addResultItem(resultItem);
            }
            ...
            for (File fileItem : uploadParam.getFileList()) {
                ...
                try {
                    ...
                    resultItem =
                            uploadFile(ftpClient, remoteDirectory, fileItem.getName(), inputStream);
    ...
    // FTP操作失败时调用，正常情况下不应该调用
    private ListResult getFailureListResultByReply(FTPClient ftpClient, ListParam listParam) {
    ...
    @Override
    public ListResult listFiles(ListParam listParam) throws FTPException {
        ListResult listResult;
        FTPClient ftpClient = null;
        try {
            ftpClient = borrowFTPClient();
            ...
            if (ftpFiles != null) {
                ...
                listResult = getFailureListResultByReply(ftpClient, listParam);
            }
        } catch (Exception e) {
    ...
    @Override
    public DownloadResult downloadFile(DownloadParam downloadParam) throws FTPException {
    ...
    @Override
    public <T> T downloadFile(FTPCallback<T> callback) throws FTPException {
        return null;
    }
    // FTP操作失败时调用，正常情况下不应该调用
    ...
    @Override
    public DeleteResult deleteFile(DeleteParam deleteParam) throws FTPException {
